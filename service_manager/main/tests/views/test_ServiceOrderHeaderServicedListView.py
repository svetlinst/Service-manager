...
class ServiceOrderHeaderServicedListViewTests(TestCase):
    USER_DATA = {
        'email': 'dev@dev.com',
        'password': 'dev',
    }
    ...
    def test_get__when_user_is_logged__expect_correct_template(self):
        ...
        response = self.client.get(reverse('service_orders_list_serviced'))

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'service_order_header/list_views/service_orders_complete.html')

    def test_get__when_user_is_not_logged__expect_to_redirect(self):
        response = self.client.get(reverse('service_orders_list_serviced'))

        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, '/accounts/login/?next=/service_orders/serviced/')

    def test_get__when_all_are_pending__expect_correct_queryset(self):
        self.client.login(**self.USER_DATA)

        response = self.client.get(reverse('service_orders_list_serviced'))
        context = response.context['object_list']
        service_orders = ServiceOrderHeader.objects.filter(Q(is_serviced=True) & Q(is_completed=False))

        self.assertEqual(len(context), len(service_orders))
        self.assertQuerysetEqual(context, service_orders, transform=lambda x: x)

    def test_get__when_one_is_serviced__expect_correct_queryset(self):
        self.client.login(**self.USER_DATA)

        service_order = ServiceOrderHeader.objects.first()
        service_order.is_serviced = True
        service_order.save()
